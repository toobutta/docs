...
The Geo-Intelligence Platform uses GitHub Actions for continuous integration and deployment. This includes automated testing, linting, security scanning, Docker image building, and deployment to staging/production environments.
...
jobs:
  ...
  deploy-staging:
    ...
    environment:
      name: staging
      url: https://api-staging.geointel.example.com
    ...
    steps:
      ...
      - name: Run smoke tests
        run: |
          ...
          curl -f https://api-staging.geointel.example.com/health || exit 1
  ...
  deploy-production:
    ...
    environment:
      name: production
      url: https://api.geointel.example.com
    ...
    steps:
      ...
      - name: Run production smoke tests
        run: |
          ...
          # Test health endpoint
          curl -f https://api.geointel.example.com/health || exit 1

          # Test authenticated endpoint (requires test token)
          # curl -f -H "Authorization: Bearer ${{ secrets.TEST_TOKEN }}" \
          #   https://api.geointel.example.com/v1/signals:query \
...
`https://github.com/your-org/geo-intelligence-platform/actions`
